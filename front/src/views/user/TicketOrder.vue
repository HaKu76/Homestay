<template>
  <el-row style="background-color: #FFFFFF;padding: 5px 30px;border-radius: 5px;">
    ...
    <el-row>
      <LineChart height="480px" tag="门票销售额（元）" @on-selected="selected" :values="values" :date="dates" />
    </el-row>
  </el-row>
</template>
...
export default {
  ...
  methods: {
    // 请求后端的金额成交数据
    selected(time) {
      this.$axios.get(`/scenicTicketOrder/daysQuery/${time}`).then(response => {
        const { data } = response;
        if (data.code === 200) {
    ...
    async fetchFreshData() {
      try {
        ...
        const response = await this.$axios.post('/scenicTicketOrder/queryScenicTicketOrder', params);
        const { data } = response;
