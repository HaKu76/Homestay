<template>
  <div class="container">
    ...
    <div>
      <p>*头像</p>
      <el-upload :on-success="handleAvatarSuccess" :show-file-list="false" action="/api/Homestay-sys/v1.0/file/upload"
    ...
    <div>
      <p>*昵称</p>
      <input v-model="userInfo.userName" class="dialog-input" placeholder="请输入"/>
    </div>
    <!-- 邮箱 -->
    <div>
      <p>*邮箱</p>
      <input v-model="userInfo.userEmail" class="dialog-input" placeholder="请输入"/>
    </div>
    <!-- 账号状态 -->
    <div>
      <div>
        <p style="font-size: 16px;">
          *账号状态
          <el-tooltip class="item" content="一经封号，不可登录，不可使用系统功能" effect="dark" placement="right">
            <i class="el-icon-info"></i>
          </el-tooltip>
        </p>
        <div>
                    <span v-if="!userInfo.isLogin" style="font-size: 14px;color: rgb(27, 156, 53);"><i
                        class="el-icon-circle-check" style="margin-right: 6px;"></i>正常</span>
          <span v-else
                style="font-size: 14px;color: rgb(230, 63, 49);text-decoration: underline;text-decoration-style: dashed;">
                        <i class="el-icon-warning-outline" style="margin-right: 6px;"></i>已被封禁</span>
        </div>
      </div>
      <div>
        <p style="font-size: 16px;">
          *留言状态
          <el-tooltip class="item" content="禁言后，互动功能受限" effect="dark" placement="right">
            <i class="el-icon-info"></i>
          </el-tooltip>
        </p>
        <div>
                    <span v-if="!userInfo.isWord" style="font-size: 14px;color: rgb(27, 156, 53);">
                        <i class="el-icon-circle-check" style="margin-right: 6px;"></i>正常</span>
          <span v-else
                style="font-size: 14px;color: rgb(230, 63, 49);text-decoration: underline;text-decoration-style: dashed;">
                        <i class="el-icon-warning-outline" style="margin-right: 6px;"></i>已被封禁</span>
        </div>
