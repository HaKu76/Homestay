<template>
  <div>
    ...
    <Editor v-model="content" :defaultConfig="editorConfig" :mode="mode"
            :style="{ height: height, overflowY: 'hidden' }" @onCreated="onCreated"/>
  </div>
</template>
<script>
import Vue from 'vue'
  ...
  data() {
    return {
      ...
      editorConfig: {
        ...
        MENU_CONF: {
          uploadImage: {
            server: '/api/Homestay-sys/v1.0/file/upload',
            fieldName: 'file',
